/**
 * @file      rs485_dual_driver_speed_test.ino
...
static constexpr uint8_t RS485_TX_ENABLE_LEVEL = HIGH;
static constexpr uint8_t RS485_RX_ENABLE_LEVEL = LOW;
...
static size_t writeBuffer(HardwareSerial &port, const uint8_t *buffer, size_t length) {
  ...
  while (written < length) {
    size_t chunk = port.write(buffer + written, length - written);
    if (chunk > 0) {
      written += chunk;
    } else {
...
static void updateOledRs485(uint32_t rx12Rate, uint32_t rx21Rate) {
  ...
  oled.clear();
  oled.drawString(0, 0, String("RS485 ") + String(RS485_BAUD) + "bps");
...
static void updateOledRs485InitFailed() {
  ...
  oled.drawString(0, 12, "Port init failed");
...
static void reportSpeed() {
  ...
  Serial.printf("[driver RS4851->RS4852] TX=%" PRIu32 " B/s RX=%" PRIu32
  ...
  Serial.printf("[driver RS4852->RS4851] TX=%" PRIu32 " B/s RX=%" PRIu32
                " B/s raw=%" PRIu32 " good=%" PRIu32 " bad=%" PRIu32 " timeout=%" PRIu32 "\n",
...
void setup() {
  ...
  delay(300);
  Serial.println();
  Serial.println("T-C5-Base dual RS485 Arduino driver speed test");
